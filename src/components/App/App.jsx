...
function App() {
  ...
  const [recordCurrentPlayer, setRecordCurrentPlayer] = useState(initialRecord);
  ...
  function crossesBody() {
    ...
    const result = bodySnake.find(
      cell => cell.x === headSnake.x && cell.y === headSnake.y,
    );
    if (result) {
      toggleStart();
      setShowModal(true);
  ...
  function gameOver() {
    if (recordCurrentPlayer < points) {
      setRecordCurrentPlayer(points);
      updatePlayer(points, id);
    }
